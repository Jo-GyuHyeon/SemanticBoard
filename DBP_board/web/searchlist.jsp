        ...
        <div class="pusher">
            ...
            <center style ="padding-top:50px"><font size='3' style ="padding-top:100px;"><h1> Q&A Board </h1></font></TD>
                ...
                <table class="ui unstackable table">
                    ...
                    <%            if (lastRow > 0) {
                            list = dao.getKeyDBAll(startRow, endRow, key, keyword);
                    ...
                    <tfoot style ="text-align: center;">
                        <tr><th colspan="3">
                                <div class="ui pagination menu" >
                                    ...
                                    %>
                                    <a class="icon item" href="./searchlist.jsp?pageNum=<%=paging.getStartPage() - 5%>&key=<%=key%>&keyword=<%=keyword%>">[이전]</a>
                                    <%
                                        }
                                        while (paging.getStartPage() <= paging.getEndPage()) {
                                            if (paging.getStartPage() == currentPage) {
                                    %>
                                    <a class="item" href="./searchlist.jsp?pageNum=<%=paging.getStartPage()%>&key=<%=key%>&keyword=<%=keyword%>">Now</a>
                                    <%
                                    } else if (paging.getStartPage() != currentPage) {
                                    %>
                                    <a class="item" href="./searchlist.jsp?pageNum=<%=paging.getStartPage()%>&key=<%=key%>&keyword=<%=keyword%>"><%=paging.getStartPage()%></a>
                                    <%
                                            }
                                    ...
                                    %>
                                    <a class="icon item" href="./searchlist.jsp?pageNum=<%=paging.getStartPage()%>&key=<%=key%>&keyword=<%=keyword%>"> > </a>
                                    <%
                                        }
                                    %>
                                </div>
